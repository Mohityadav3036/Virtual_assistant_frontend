 ...
 const [ error,setError] = useState("")
  ...
  return (
    <div className='w-full h-[100vh]  bg-cover flex justify-center items-center' style={{backgroundImage:`url(${bgimage})`}}>
          ...
          <form className=' w-[90%] h-[600px] max-w-[500px] bg-[#00000062] backdrop-blur shadow-lg shadow-black flex flex-col items-center justify-center gap-[20px] p-[20px]' onSubmit={handleSignup}>
          <h1 className=' text-white text-[30px] font-semibold mb-[30px]'>Register to <span className=' text-blue-950 '>Virtual Assistant</span></h1>
                         
                  <input type='text' placeholder='Enter your Name' className=' w-full h-[60px] text-xl outline-none border-2 border-white rounded-xl bg-transparent text-white placeholder-gray-300 px-[20px] py-[10px]' required onChange={(e)=>setName(e.target.value)} value={name}/>
                   ...
                   <button type='submit'  className=' mt-[30px] min-w-[150px] bg-white h-[60px] rounded-xl text-xl text-black font-semibold' disabled={loading}> {loading?"Loading...":"Sign Up"}</button>

              <p className=' text-white'>Already have an account <span className=' text-blue-300 cursor-pointer' onClick={()=>navigate("/signin")}>Sign In</span></p>
